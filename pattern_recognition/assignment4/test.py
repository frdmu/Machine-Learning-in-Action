...
def main():
	...
	predict_label = np.array(category)
	test_label = np.loadtxt("TestLabels.csv", dtype=np.float, delimiter=",")
	acc = np.sum(predict_label == test_label) / predict_label.shape[0]
